...
class EmployeeDAOTest {
	...
	void testEmployeeDAO() {
		...
		when(dao.get(id)).thenReturn(new Employee(id,"ABC",20000));
	...
	void testEmployeeDAOGetAll() {
		...
		when(emps.get(0)).thenReturn(new Employee(1,"Def", 1000));
		when(emps.get(1)).thenReturn(new Employee(2,"Xyz", 1000));
