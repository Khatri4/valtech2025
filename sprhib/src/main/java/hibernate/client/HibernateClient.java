...
//				.addAnnotatedClass(StudentId.class)
				.buildSessionFactory();
...
//		ses.createQuery("FROM Customer c WHERE c.age > ?").setInteger(0, 25).list().stream().forEach(t-> System.out.println(t));
		ses.createQuery("SELECT t.amount FROM Tx t JOIN t.account.customers c WHERE c.age < ?").setInteger(0, 25).list().stream().forEach(t-> System.out.println(t));
		
		
//		StudentId id = (StudentId)ses.save(new Student(new StudentId(1,2025), "ABC", "Def", "CS", 9099909090L
//				,new StudentAddress("SomeWhere", "AHM", 382330)
...
//		ses.update(car);
//		ses.flush(); // dont call session on the flush on your own...
//		System.out.println(car);
//		System.out.println(car.getClass().getName());
		
//		Account acc = (Account) ses.load(Account.class, 1L);
//		Customer cus = (Customer) ses.load(Customer.class, 1);
//		
//		Account acc1 = new Account(50000, "CA");
//		Account acc2 = new Account(40000, "SB");
//		
